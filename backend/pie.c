...
enum Pie_Option
{
  ...
  OPT_GEOMETRY_GROUP, /* 4 */
  OPT_TL_X,	 /* 5 */		/* top-left x */
  OPT_TL_Y,	 /* 6 */		/* top-left y */
  OPT_BR_X,	 /* 7 */		/* bottom-right x */
  OPT_BR_Y,	 /* 8 */		/* bottom-right y */

  /* ------------------------------------------- */

  OPT_ENHANCEMENT_GROUP, /* 9 */

  OPT_HALFTONE_PATTERN,  /* 10 */
  OPT_SPEED,             /* 11 */
  OPT_THRESHOLD,         /* 12 */

  OPT_GAMMA_VECTOR,      /* 13 */
  OPT_GAMMA_VECTOR_R,    /* 14 */
  OPT_GAMMA_VECTOR_G,    /* 15 */
  OPT_GAMMA_VECTOR_B,    /* 16 */

  /* ------------------------------------------- */

  OPT_ADVANCED_GROUP,    /* 17 */
  OPT_PREVIEW,           /* 18 */
...
/*
 * pie_ieee1284_control_init - set IEEE1284 control to init
 *
 */
static SANE_Status
pie_ieee1284_control_init(int fd)
{
  SANE_Int status;
  static SANE_Byte init[1] = { C1284_NINIT };
  DBG (DBG_lowlevel, "\t\tpie_ieee1284_control_init\n");
  usleep(3000);
...
/*
 * pie_ieee1284_control_strobe - issue IEEE1284 strobe
 *
 */
static SANE_Status
pie_ieee1284_control_strobe(int fd)
{
  ...
  if (status == SANE_STATUS_GOOD)
    {
      status = pie_ieee1284_control_init(fd);
    }
  return status;
}

/*
 * pie_ieee1284_command_write - write single command byte to IEEE1284 command register
 *
 */
static SANE_Status
pie_ieee1284_command_write(int fd, SANE_Byte cmd)
{
  SANE_Int status;
  static SANE_Byte buf[1];
  buf[0] = cmd;
  DBG (DBG_lowlevel, "\t\tpie_ieee1284_command_write\n");
  usleep(2000);
  status = sanei_usb_control_msg (fd, PIE_USB_WRITE, PIE_USB_REQ_ONE,
...
pie_ieee1284_command_prefix(int fd)
{
  ...
  DBG (DBG_lowlevel, "\t\tpie_ieee1284_command_prefix\n");
...
{
  SANE_Int status;
  DBG (DBG_lowlevel, "\t\tpie_ieee1284_command\n");

  while (1)
    {
      status = pie_ieee1284_command_prefix (fd);
      if (status != SANE_STATUS_GOOD)
        break;
      status = pie_ieee1284_command_write (fd, command);
      if (status != SANE_STATUS_GOOD)
        break;
      status = pie_ieee1284_control_strobe (fd);
      if (status != SANE_STATUS_GOOD)
        break;
      status = pie_ieee1284_command_write (fd, 0xff);
      if (status != SANE_STATUS_GOOD)
        break;
      break;
    }
  ...
  DBG (DBG_lowlevel, "\t\tpie_ieee1284_command returns %d\n", status);
  return status;
}
...
{
  DBG (DBG_lowlevel, "\t\tpie_ieee1284_addr\n");
...
{
  DBG (DBG_lowlevel, "\t\tpie_ieee1284_reset\n");
  return pie_ieee1284_command (fd, PIE_IEEE1284_RESET);
}


/*
 * pie_usb_scsi_status_read - read single status byte from SCSI register
 *
 * @returns: PIE_SCSI_STATE_xxx
 */
static int
pie_usb_scsi_status_read(int fd)
{
  ...
  DBG (DBG_lowlevel, "\tpie_usb_scsi_status_read\n");
  usleep(2000);

  status = sanei_usb_control_msg (fd, PIE_USB_READ, PIE_USB_REQ_ONE,
				PIE_USB_SCSI_STATUS, 0, 1, &state);
  if (status != SANE_STATUS_GOOD)
    {
...
/*
 * write scsi cmd/data via usb
 *
 */

static int
pie_usb_scsi_cmd_out(int fd, unsigned char *src, size_t src_size)
{
  SANE_Int status;
  int scsi_status;
  
  DBG (DBG_proc, "\tpie_usb_scsi_cmd_out: %d bytes\n", (int)src_size);
  while (src_size > 0)
    {
...
static SANE_Status
pie_usb_scsi_cmd(int fd, unsigned char * src, size_t src_size, unsigned char * dst, size_t * dst_size)
{
  SANE_Int status;
  SANE_Int data_size;
  int scsi_status;
  ...
  DBG (DBG_proc, "\tpie_usb_scsi_cmd: len %d, cmd 0x%02x, data size 0x%02x\n", (int)src_size, *src, data_size);
  DBG_DUMP (DBG_dump, src, src_size);
  if (src_size < PIE_SCSI_CMD_LEN)
    {
      DBG (DBG_error, "pie_usb_scsi_cmd: scsi cmd too short\n");
  ...
  do
    {
      ...
      scsi_status = pie_usb_scsi_cmd_out (fd, src, PIE_SCSI_CMD_LEN);
	  ...
	  if (src_size > PIE_SCSI_CMD_LEN)
	    {
	      scsi_status = pie_usb_scsi_cmd_out (fd, src+PIE_SCSI_CMD_LEN, src_size-PIE_SCSI_CMD_LEN );
	    }
	}
      if (scsi_status == PIE_SCSI_STATE_WAIT)
        {
	  switch (*src)
	    {
  ...
  while (scsi_status == PIE_SCSI_STATE_WAIT);

  /*
   * expecting data return ?
   * -> write expected size, the bulk_in
   *
   */

  if (scsi_status == PIE_SCSI_STATE_LEN)
    {
      static SANE_Byte sizebuf[8] = { 0 };

      /* read request, send expected length */
      if (data_size <= 0)
        {
          DBG (DBG_error, "pie_usb_scsi_cmd: scsi status is PIE_SCSI_STATE_LEN, but expected_size is %d\n", data_size);
          return SANE_STATUS_CANCELLED;
        }

      if (!dst || !dst_size || *dst_size == 0)
        {
          DBG (DBG_error, "pie_usb_scsi_cmd: Expecting data but no dest buffer available\n");
          return SANE_STATUS_INVAL;
        }

      /* write size buffer to SCSI register */
      sizebuf[4] = data_size;
      usleep(2000);
      ...
      DBG (DBG_sane_info, "\tread_bulk: %d bytes:\n", (int)*dst_size);
      DBG_DUMP (DBG_sane_info, dst, (int)*dst_size);
...
/*
 * Issue SCSI write followed by data
 *
 */

static SANE_Status
pie_scsi_usb_write( Pie_Scanner *scanner, SANE_Byte *buf, size_t buf_size )
{
  SANE_Status status;
  
  DBG (DBG_proc, "\tpie_scsi_usb_write: %d bytes:\n", (int)buf_size);
  DBG_DUMP (DBG_sane_info, buf, (int)buf_size);

  /* scsi write cmd */
  set_write_length (swrite.cmd, buf_size);
  status = pie_usb_scsi_cmd (scanner->sfd, swrite.cmd, swrite.size, NULL, NULL);
  if (status != SANE_STATUS_GOOD)
    {
      DBG (DBG_error, "pie_scsi_usb_write: Can't issue 'write' cmd\n");
      return status;
    }

  /* scsi write data */
  status = pie_usb_scsi_cmd (scanner->sfd, buf, buf_size, NULL, NULL);
  if (status != SANE_STATUS_GOOD)
    {
      DBG (DBG_error, "\tpie_scsi_usb_write: Can't write data\n");
    }
  return status;
}
...
static void
pie_device_close(Pie_Device * dev, int sfd)
{
  DBG (DBG_proc, "device_close\n");
...
static SANE_Status
pie_scsi_cmd (int bus, int fd, void * src, size_t src_size, void * dst, size_t * dst_size)
{
  if (bus == PIE_BUS_SCSI)
...
pie_do_inquiry (int bus, int sfd)
{
  size_t size;
  SANE_Status status;
  unsigned char *buffer;
#define DO_INQUIRY_SIZE 256

  DBG (DBG_proc, "do_inquiry\n");
  
  buffer = (unsigned char *)calloc(1, DO_INQUIRY_SIZE);
  if (!buffer)
    return NULL;
  ...
  status = pie_scsi_cmd (bus, sfd, inquiry.cmd, inquiry.size, buffer, &size);
  if (status)
    {
  ...
  if (size > DO_INQUIRY_SIZE)
    {
      DBG (DBG_error, "pie_do_inquiry: inquire size %d does not fit into buffer\n", (int)size);
    }
  ...
  return buffer;
#undef DO_INQUIRY_SIZE
}

/* ----------------------------- PIE READ GAIN OFFSET ---------------------------- */

static void
pie_read_gain_offset (Pie_Scanner *scanner)
{
  size_t size;
  SANE_Status status;
  unsigned char *buffer;

  DBG (DBG_proc, "pie_read_gain_offset\n");
  ...
  if (status != SANE_STATUS_GOOD)
    {
      DBG (DBG_error, "pie_read_gain_offset: scanner not ready\n");
      return;
    }

  size = read_gain_offset.cmd[4];
  buffer = (unsigned char *)calloc(1, size);
  if (!buffer)
    {
      DBG (DBG_error, "pie_read_gain_offset: calloc(%d) failed\n", (int)size);
      return;
    }
  status = pie_scsi_cmd (scanner->device->bus, scanner->sfd, read_gain_offset.cmd, read_gain_offset.size, buffer, &size);
  if (status)
    {
      DBG (DBG_error, "pie_read_gain_offset: command returned status %s\n",
	   sane_strstatus (status));
    }
/* FIXME: evaluate gain offset */
...
pie_identify_scanner (Pie_Device * dev, int sfd)
{
  ...
  inquiry_block = pie_do_inquiry (dev->bus, sfd);	/* get inquiry */
  if (!inquiry_block)
    {
      /* allow two attempts */
      sleep(2);
...
pie_get_cal_info (Pie_Device * dev, int sfd)
{
  ...
  if (!(dev->inquiry_scan_capability & INQ_CAP_EXT_CAL))
    {
      DBG (DBG_info, "pie_get_cal_info: not a capability (0x%x) vs 0x%x\n", dev->inquiry_scan_capability, INQ_CAP_EXT_CAL);
      return;
    }
  size = 6;
...
static SANE_Status
attach_usb (const char *usbname)
{
  DBG (DBG_sane_proc, "attach_usb: %s\n", usbname);
...
attach_scanner_usb (const char *devicename, Pie_Device * dev, int *sfd)
{
  ...
  usbName = NULL;
  if (sanei_usb_find_devices (vendor, product, attach_usb) != SANE_STATUS_GOOD)
    {
  ...
  if (!usbName)
    {
      DBG (DBG_error, "attach_scanner: sanei_usb_find_devices no device\n");
      return SANE_STATUS_INVAL;
    }
...
static SANE_Status
attach_scanner_scsi (const char *devicename, Pie_Device * dev, int *sfd)
{
  int bufsize;

  DBG (DBG_sane_proc, "attach_scanner_scsi: %s\n", devicename);
...
attach_scanner (const char *devicename, Pie_Device ** devp)
{
  ...
  if (strncmp(devicename, "usb ", 4) == 0)
    {
      SANE_Status status;
      status = attach_scanner_usb (devicename, dev, &sfd);
      if (status != SANE_STATUS_GOOD)
        {
          free (dev);
          return status;
    ...
    {
      SANE_Status status;
      status = attach_scanner_scsi (devicename, dev, &sfd);
      if (status != SANE_STATUS_GOOD)
        {
          free (dev);
          return status;
...
/*  pie_wait_device (dev, sfd); */
  ...
  pie_device_close (dev, sfd);
  dev->sane.type = (dev->bus == PIE_BUS_SCSI) ? "flatbed scanner" : "slide scanner";
  ...
  DBG (DBG_sane_proc, "attach_scanner: first_dev @ %p\n", (void *)first_dev);
  DBG (DBG_sane_proc, "attach_scanner: first_dev = %s\n", first_dev->devicename);
  return SANE_STATUS_GOOD;
}
...
static SANE_Status
pie_command_17 (Pie_Scanner * scanner)
{
  unsigned char buffer[128];
  size_t size;
  SANE_Status status;
  ...
  if (status)
    {
      DBG (DBG_error,
	   "pie_command_17: write command returned status %s\n",
	   sane_strstatus (status));
    }
...
static SANE_Status
pie_set_gain_offset (Pie_Scanner * scanner)
{
  unsigned char buffer[128];
  size_t size;
  SANE_Status status;
  unsigned char data[] = { 0x43, 0x10, 0x11, 0x10, 0x1c, 0x10, 0x52, 0x4e, 0x6a, 0x00, 0x00, 0x00, 0x35, 0x2d, 0x2e, 0x05, 0x00, 0x00, 0x04, 0x10, 0x4e, 0x00, 0x2c, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };

  if (scanner->device->bus != PIE_BUS_USB)
    return SANE_STATUS_GOOD;

  DBG (DBG_proc, "pie_set_gain_offset\n");

  size = sizeof(data);

  set_write_length (set_gain_offset.cmd, size);

  memcpy (buffer, set_gain_offset.cmd, set_gain_offset.size);

  memcpy (buffer + set_gain_offset.size, data, size);
  ...
  if (status != SANE_STATUS_GOOD)
    {
      DBG (DBG_error,
	   "pie_set_gain_offset: write command returned status %s\n",
	   sane_strstatus (status));
    }
  else
    {
...
static SANE_Status
pie_slide_control (Pie_Scanner * scanner, SANE_Byte cmd) /*, SANE_Byte val) */
{
  unsigned char buffer[128];
  size_t size;
  SANE_Status status;
  ...
  DBG (DBG_proc, "pie_slide_control 0x%02x\n", cmd);

  size = sizeof(data);

  set_write_length (slide_ctrl.cmd, size);

  memcpy (buffer, slide_ctrl.cmd, slide_ctrl.size);

  memcpy (buffer + slide_ctrl.size, data, size);
  buffer[slide_ctrl.size] = cmd;
/*  buffer[slide_ctrl.size+1] = val;*/

  status =
    pie_scsi_cmd (scanner->device->bus, scanner->sfd, buffer, slide_ctrl.size + size, NULL, NULL);
  if (status != SANE_STATUS_GOOD)
    {
      DBG (DBG_error,
	   "pie_slide_control: write command returned status %s\n",
	   sane_strstatus (status));
    }
  else
    {
...
static SANE_Status
pie_wait_device (Pie_Device *dev, int sfd)
{
  SANE_Status status;
  int cnt = 0;
  ...
  do
      ...
      if (cnt > 100)		/* maximal 100 * 0.5 sec = 50 sec */
	{
      ...
      /* test unit ready */
      status =
	pie_scsi_cmd (dev->bus, sfd, test_unit_ready.cmd,
...
{
  return pie_wait_device (scanner->device, scanner->sfd);
}
...
pie_get_params (Pie_Scanner * scanner)
{
  ...
  if (scanner->device->bus == PIE_BUS_USB)
    size = 18;
  else if (scanner->device->inquiry_image_format & INQ_IMG_FMT_OKLINE)
    size = 16;
  else
    size = 14;
...
pie_reader_process_indexed (Pie_Scanner * scanner, FILE * fp)
{
  ...
  DBG (DBG_proc, "pie_reader_process_indexed\n");
...
pie_reader_process (Pie_Scanner * scanner, FILE * fp)
{
  ...
  DBG (DBG_proc, "pie_reader_process\n");
...
reader_process ( void *data )	/* executed as a child process */
{
  ...
  if (!fp)
    {
      DBG (DBG_error, "fdopen(%d) failed: %s\n", scanner->reader_fds, strerror(errno));
      return SANE_STATUS_IO_ERROR;
    }
  ...
  if (scanner->sfd >= 0)
    {
      ...
      pie_device_close (scanner->device, scanner->sfd);
      scanner->sfd = -1;
    }
...
sane_init (SANE_Int * version_code, SANE_Auth_Callback __sane_unused__ authorize)
{
  ...
  if (!fp)
    {
      DBG (DBG_warning, "pie.conf not found, using SCSI /dev/scanner\n");
...
sane_open (SANE_String_Const devicename, SANE_Handle * handle)
{
  ...
  if (devicename[0])		/* search for devicename */
  ...
  else
    {
      ...
      DBG (DBG_sane_init, "sane_open: using first_dev(%p:%s)\n", (void *)dev, dev?dev->devicename:"");
    }
  ...
  *handle = scanner;
  ...
  DBG (DBG_sane_init, "sane_open: scanner(%p:%s)\n", (void *)scanner, scanner->device->devicename);
  return SANE_STATUS_GOOD;
}
		     ...
		     void *val, SANE_Int * info)
{
  ...
  DBG (DBG_proc, "sane_control_option opt %d(%s), act %d\n", (int)option, (option < NUM_OPTIONS) ? ((scanner->opt[option].name)?scanner->opt[option].name:"(null)") : "*ERR*", (int)action);
...
static SANE_Status
pie_scsi_sane_start(Pie_Scanner *scanner)
{
#ifdef HAVE_SANEI_SCSI_OPEN_EXTENDED
  ...
  if (sanei_scsi_open_extended
    ...
    {
      DBG (DBG_error, "pie_scsi_sane_start: open failed\n");
      return SANE_STATUS_INVAL;
    }
  ...
  if (scsi_bufsize < 32768)	/* < 32KB */
    {
      DBG (DBG_error,
           "pie_scsi_sane_start: sanei_scsi_open_extended returned too small scsi buffer (%d)\n",
           scsi_bufsize);
      sanei_scsi_close (scanner->sfd);
      return SANE_STATUS_NO_MEM;
    }
  DBG (DBG_info,
       "pie_scsi_sane_start: sanei_scsi_open_extended returned scsi buffer size = %d\n",
       scsi_bufsize);
  ...
  if (sanei_scsi_open
    ...
    {
      DBG (DBG_error, "pie_scsi_sane_start: open of %s failed:\n",
...
sane_start (SANE_Handle handle)
{
  ...
  if (scanner->sfd < 0)		/* first call, don`t run this routine again on multi frame or multi image scan */
    {
      if (scanner->device->bus == PIE_BUS_SCSI)
        {
          status = pie_scsi_sane_start (scanner);
        }
      else
        {
  ...
  pie_command_17 (scanner); /* powerslide: ??? */
  pie_read_gain_offset (scanner);

  status = pie_do_cal (scanner);
  if (status)
    return status;

  pie_set_gain_offset (scanner);
  ...
  if (scanner->reader_pid == -1)
    {
      DBG (DBG_error, "sane_start: sanei_thread_begin failed (%s)\n",
             strerror (errno));
      return SANE_STATUS_NO_MEM;
    }
  ...
  if (sanei_thread_is_forked ())
    {
      DBG (DBG_info, "sane_start: (forked) sanei_thread_begin pid %d\n", scanner->reader_pid);
...
static SANE_Status
pie_copy_data (Pie_Scanner *scanner)
{
  size_t size, orig_size;
  SANE_Status status;
  unsigned char *buffer;

  DBG (DBG_proc, "pie_copy_data\n");

  orig_size = size = copy_data.cmd[4] + (copy_data.cmd[3]<<8);
  buffer = (unsigned char *)calloc(1, size);
  if (!buffer)
    {
      DBG (DBG_error, "pie_copy_data: calloc(%d) failed\n", (int)size);
      return SANE_STATUS_NO_MEM;
    }
  status = pie_scsi_cmd (scanner->device->bus, scanner->sfd, copy_data.cmd, copy_data.size, buffer, &size);
  if (status != SANE_STATUS_GOOD)
    {
      DBG (DBG_error, "pie_copy_data: copy command returned status %s\n",
	   sane_strstatus (status));
      free(buffer);
      return status;
    }
  ...
  free(buffer);
  orig_size = size = read_reverse.cmd[4];
  buffer = (unsigned char *)calloc(1, size);
  if (!buffer)
    {
      DBG (DBG_error, "pie_copy_data: calloc(%d) failed\n", (int)size);
  ...
  status = pie_scsi_cmd (scanner->device->bus, scanner->sfd, read_reverse.cmd, read_reverse.size, buffer, &size);
  if (status != SANE_STATUS_GOOD)
    {
      DBG (DBG_error, "pie_copy_data: read reverse command returned status %s\n",
	   sane_strstatus (status));
      free(buffer);
      return status;
    }
